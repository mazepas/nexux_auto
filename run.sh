...
# -----------------------------------------------------------------------------
# 4) Automatically agree to the Nexus Beta Terms of Use and create a node-id file.
#    This prevents the prompt asking for user agreement.
# -----------------------------------------------------------------------------
# Touch the node-id file to skip the agreement prompt (simulating agreement).
touch "$NEXUS_HOME/node-id"
...
# -----------------------------------------------------------------------------
# 8) Run the Rust CLI in interactive mode, but with automated confirmation.
#    This simulates user input to bypass the prompts.
# -----------------------------------------------------------------------------
(
  cd "$REPO_PATH/clients/cli" || exit
  echo -n "y" | cargo run -r -- start --env beta
) < /dev/tty
# -----------------------------------------------------------------------------
